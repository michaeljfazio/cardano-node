...
instance Show (Witness era) where
    ...
    showsPrec _p (ShelleyDatumWitness _ _) = error "TODO: Jordan"
    showsPrec _p (ShelleyRedeemerBudget _ _) = error "TODO: Jordan"
...
instance IsCardanoEra era => SerialiseAsCBOR (Witness era) where
    ...
    serialiseToCBOR (ShelleyDatumWitness _ _) = error "TODO:Jordan"

    serialiseToCBOR (ShelleyRedeemerBudget _ _) = error "TODO:Jordan"
...
getTxWitnesses (ShelleyTx era tx) =
  ...
  where
    ...
    getAlonzoTxWitnesses Alonzo.Tx {
     ...
     ++ map (ShelleyKeyWitness       era) (Set.elems addrWits)
     ++ map (ShelleyScriptWitness    era) (Map.elems msigWits)
     ++ map (ShelleyDatumWitness     era) (Map.elems datWits)
