...
getBlockTxs (ShelleyBlock shelleyEra Consensus.ShelleyBlock{Consensus.shelleyBlockRaw})
  ...
  where
    ...
    go = case shelleyBlockRaw of Shelley.Block _header (Shelley.TxSeq txs) -> [ShelleyTx shelleyEra (error "TODO") | _x <- toList txs]
    --TODO: Jordan - Consensus needs use the Core shelley api for blocks
-- ----------------------------------------------------------------------------
